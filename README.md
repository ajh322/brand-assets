...
저장소를 가볍게 두려는 것이고, 그 판단은 지금도 유효하다.

⚠️ **"jsDelivr 가 50MB 까지만 서빙한다" 는 말이 여기 적혀 있었는데 사실이 아니다.**
2026-09-21 실측: 저장소가 **309MB** 인 상태에서 루딕스·SHL 22개 제품 이미지를
전부 받아봤고 **22/22 정상(200)** 이었다. 8.7MB 짜리 webp 도 그대로 나온다.

**그래서 크기 때문에 파일을 지우지 마라.** 이 문서를 읽고 "50MB 를 넘었으니
정리해야겠다" 며 이미지를 지우면, **이미 발행된 글과 상세페이지가 그 URL 을
박아두고 있어서 한꺼번에 깨진다.** 지우는 것이 오히려 사고다.
...
0. **큰 원본(자르기 전 통이미지)은 `brand-assets-source` 로 보낸다.**
   이 저장소는 링크로 가져다 쓰는 것만 둔다. 크기 때문에 **기존 파일을
   지우지는 않는다** — 발행된 글이 그 URL 을 쓰고 있다
